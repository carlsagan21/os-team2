...
{
	return container_of(se, struct task_struct, wrr);
}
...
static inline struct wrr_rq *wrr_rq_of_wrr(struct sched_wrr_entity *wrr)
{
	struct task_struct *p = wrr_task_of(wrr);
...
static void
enqueue_wrr_entity(struct wrr_rq *wrr_rq, struct sched_wrr_entity *wrr, int flags)
{
	list_add_tail(&wrr->run_list, &wrr_rq->run_queue);
...
static struct sched_wrr_entity *pick_next_wrr_entity(struct wrr_rq *wrr_rq)
{
	...
	struct sched_wrr_entity *wrr = list_first_entry_or_null(&wrr_rq->run_queue, struct sched_wrr_entity, run_list);
...
static void requeue_task_wrr(struct rq *rq, struct task_struct *p, int head)
{
	...
	//
	// list_move_tail(&wrr->run_list, &wrr_rq->run_list);
}
...
static void update_curr_wrr(struct rq *rq){
	...
	wrr = list_first_entry_or_null(&wrr_rq->run_queue, struct sched_wrr_entity, run_list);

	if (wrr != NULL) {
		curr = wrr_task_of(wrr);
...
enqueue_task_wrr(struct rq *rq, struct task_struct *p, int flags)
{
	...
	enqueue_wrr_entity(wrr_rq, wrr, 0);
...
static struct task_struct *pick_next_task_wrr(struct rq *rq)
{
	...
	wrr = pick_next_wrr_entity(wrr_rq);
...
#ifdef CONFIG_SCHED_DEBUG
	// printk(KERN_DEBUG "[soo] wrr_func pick_next_task_wrr2: %d", wrr_task_of(wrr)->pid);
	// print_wrr_list(&rq->wrr);
#endif
	return p;
}
...
static void put_prev_task_wrr(struct rq *rq, struct task_struct *prev)
{
	// struct wrr_rq *wrr_rq = &rq->wrr;
	// wrr_rq->curr = NULL;
	//TODO list_move_tail(&prev->wrr.run_list, &wrr_rq->run_list);
	// requeue_task_wrr(rq, rq->curr, 0);
...
static void switched_to_wrr(struct rq *rq, struct task_struct *p)
{
	...
	p->wrr.time_slice = DEFAULT_WEIGHT * WRR_TIMESLICE;
#ifdef CONFIG_SCHED_DEBUG
...
//  list_for_each_entry(wrr, &wrr_rq->run_list, run_list) {
//          p = wrr_task_of(wrr);
//          printk(KERN_DEBUG "[soo] print_wrr_list task: %d, %d, %d, %d", p->pid, p->wrr.weight, p->wrr.time_slice, HZ);
//  }
